    ...
    void (async () => {
      try {
        ...
        if (!current) {
          ...
          setError("");
          document.title = `${current.title} | BangHeroes`;
          // Keep URL on the canonical title-slug path when opened via /watch/{id}
          const canonical = filmWatchPath(current);
          if (
            typeof window !== "undefined" &&
            window.location.pathname.replace(/\/$/, "") !== canonical
          ) {
              ...
              <div className="relative aspect-video w-full">
                ...
                <div className="absolute inset-0 flex flex-col items-center justify-center gap-4 p-6 text-center">
                  <p className="max-w-md text-sm text-cinema-muted">
                    This title streams on the original site. Continue through a
                    short ad page to watch.
                  </p>
                  <Link
